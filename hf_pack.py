...
FLAGS, FLAGS_DEF = utils.define_flags_with_default(
    ...
    dataset='news',
    #
    include_feedback='p,n,pn,np', # p: positive only, n: negative only, pn: positive-negative, np: negative-positive, aux: auxiliary
    #
...
def process_hh_cn(example):
    all_output = []

    def pack_data(data, is_positive, output, fields=[]):
        messages = [d['text'] for d in data['context']]
        if is_positive:
            messages += [data['chosen']['text']]
        else:
            messages += [data['rejected']['text']]
...
def process_news(example):
    all_output = []
    pos_idx = int(example['better_response_id'])
    neg_idx = 1 - pos_idx

    def pack_data(idx, is_positive, output, fields, use_user_input=True):
        if use_user_input:
            output['{}_{}'.format(FLAGS.user_data_id, idx)] = f"{example['instruction']}\n{example['input']}"
            user_input_fields = '{}+{}_{}+'.format(FLAGS.user_field_id, FLAGS.user_data_id, idx)
        else:
            user_input_fields = ''

        answer = example[f'response_{idx}'].lstrip()
        if is_positive:
            output['{}_{}'.format(FLAGS.gpt_data_id, idx)] = f"{answer}"
            fields.append('[{}{}]'.format(user_input_fields, FLAGS.gpt_field_id))
            fields.append('{}_{}'.format(FLAGS.gpt_data_id, idx))
    ...
    if 'aux' in FLAGS.include_feedback.split(','):
        for gpt_marker, gpt_bad_marker in zip(NEWS['good'], NEWS['bad']):
            format_fn(gpt_marker, gpt_bad_marker)
    else:
        gpt_marker = FLAGS.gpt_marker
        gpt_bad_marker = FLAGS.gpt_bad_marker
...
NEWS = {
    'good': [
        'The following is a better news article.',
        'The following is a more accurate news article.',
        'The following is a more correct news article.',
        'Generate a better news article.',
        'Generate a more accurate news article.',
        'Generate a more correct news article.',
    ],
    'bad': [
        'The following is a worse news article.',
        'The following is a less accurate news article.',
        'The following is a less correct news article.',
        'Generate a worse news article.',
        'Generate a less accurate news article.',
        'Generate a less correct news article.',
    ],
}
        ...
        if dataset == 'dialogue': #https://huggingface.co/datasets/Anthropic/hh-rlhf
            ...
            with open(all_eval_file, 'w') as fout:
                for example in eval_data:
                    ...
                    for output in all_output:
                        fout.write(json.dumps(output) + '\n')
        if dataset == 'hh_cn': #https://huggingface.co/datasets/Anthropic/hh-rlhf
            train_data = load_dataset('json', data_dir="/data/chuxiong/hh_rlhf_cn", split='train')
            train_output_file = f"{FLAGS.output_dir}/hh_cn_train_{FLAGS.include_feedback}.jsonl"
            with open(train_output_file, 'w') as fout:
                for example in train_data:
                    all_output = process_hh_cn(example)
                    for output in all_output:
            ...
            eval_data = load_dataset('json', data_dir="/data/chuxiong/hh_rlhf_cn", split='test')
            eval_output_file = f"{FLAGS.output_dir}/hh_cn_eval_{FLAGS.include_feedback}.jsonl"
            with open(eval_output_file, 'w') as fout:
                for example in eval_data:
                    all_output = process_hh_cn(example)
                    for output in all_output:
            ...
            train_data = load_dataset('json', data_files='/d1/data/chuxiong/preference_data_final_sampled.jsonl',split='train')
            train_output_file = f"{FLAGS.output_dir}/news_train_{FLAGS.include_feedback}.jsonl"
            with open(train_output_file, 'w') as fout:
                for example in train_data:
                    all_output = process_news(example)
                    for output in all_output:
                        fout.write(json.dumps(output, ensure_ascii=False) + '\n')
            with open(all_train_file, 'w') as fout:
                for example in train_data:
